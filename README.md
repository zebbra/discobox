...
- **Wireless APs** — during a WLC's sync, updates each AP it reports (found by serial, then name): DeviceType, serial, `os_version`, an optional `controller` link to the WLC (the WLC then gets a `## Wireless controller` comments block linking the list of its APs), a `## Wireless` block in the comments (controller, location, uplink, MACs, radios; no DHCP IP or timestamp, so it only changes when something real does; text outside its markers is never touched), a `GigabitEthernet0` interface with the Ethernet MAC, and a `Dot11Radio<slot>` per reported radio (devicetype-library names, so template interfaces are adopted), the radio base MAC on the first radio only (the WLC reports the same one for every slot). The WLC's per-AP radio "ports" are not synced as its own interfaces. Every other AP interface (import placeholders like `main`/`Vlan2`, ports from a wrong DeviceType template) is removed on every WLC sync unless it has a cable, an IP or another source's ownership, and so are empty device/module bays (`aps.prune_interfaces`, default on). Once an AP's model resolves to a DeviceType, a `fixme-model` tag is removed from it (`aps.type_confirmed_untag`)
- **WLC HA pairs** — a Cisco 9800 HA SSO pair (ENTITY-MIB root "Multi Chassis System") becomes one Netbox device per unit in a Virtual Chassis, by chassis position (chassis 1 = the controller's own device, chassis 2 = `<name>-2`), so roles can switch over without devices swapping. A missing `-2` peer is created (role/site/tenant/status/type from the controller, serial from its chassis; location/rack left empty) with a `## HA peer` comments block that flags the missing location until it's set. Each unit gets its own chassis serial (the controller's combined "A B" serial is never written); chassis module bays from the earlier modelling are removed
...
| Chassis count (traditional stack only) | `custom_fields.stack_members` |
